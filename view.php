...
$PAGE->set_url('/mod/moodlecst/view.php', array('id' => $cm->id));
...
//$PAGE->requires->js("/mod/moodlecst/js/somejs.js");
...
if(has_capability('mod/moodlecst:preview',$modulecontext)){
	echo $renderer->header($moduleinstance, $cm, $mode, null, get_string('view', MOD_MOODLECST_LANG));
}else{
...
if($moduleinstance->maxattempts > 0){
	$attempts =  $DB->get_records(MOD_MOODLECST_USERTABLE,array('userid'=>$USER->id, MOD_MOODLECST_MODNAME.'id'=>$moduleinstance->id));
	if($attempts && count($attempts)<$moduleinstance->maxattempts){
		echo get_string("exceededattempts",MOD_MOODLECST_LANG,$moduleinstance->maxattempts);
	}
}
...
switch($moduleinstance->mode){
	case MOD_MOODLECST_MODESTUDENTSTUDENT:
		...
		echo $renderer->show_student_newsessionlink($cm,$caption,$moduleinstance);
		break;
